    ...
    while cap.isOpened():
        ...
        if model_type == "yolo":
            ...
            for box in boxes:
                ...
                # Mejorar la visibilidad de la etiqueta con un fondo y contorno
                # Obtener el tamaño del texto para crear el rectángulo de fondo
                (text_width, text_height), baseline = cv2.getTextSize(
                    label, cv2.FONT_HERSHEY_SIMPLEX, 0.9, 2)
                
                # Coordenadas para el rectángulo de fondo
                text_x = x1
                text_y = y1 - 10
                
                # Dibujar un rectángulo negro como fondo del texto
                cv2.rectangle(
                    frame_with_results, 
                    (text_x, text_y - text_height - 5), 
                    (text_x + text_width, text_y + 5), 
                    (0, 0, 0), 
                    -1  # -1 para rellenar el rectángulo
                )
                
                # Dibujar el texto con contorno (efecto de texto más visible)
                # Primero dibujar el texto con un borde negro (contorno)
                cv2.putText(frame_with_results, label, (text_x, text_y), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 0), 4)
                # Luego dibujar el texto en color encima
                cv2.putText(frame_with_results, label, (text_x, text_y), 
...
def visualize_results_without_probs(frame, outputs):
    ...
    if isinstance(outputs, torch.Tensor):
        ...
        # Dibujar la etiqueta de clase con mejor visibilidad
        ...
        # Crear un rectángulo negro como fondo
        cv2.rectangle(
            frame,
            (10, 30 - text_height - 5),
            (10 + text_width, 30 + 5),
        ...
        # Dibujar el texto con contorno (efecto de texto más visible)
        # Primero dibujar el texto con un borde negro más grueso
...
if __name__ == "__main__":
    ...
    output_path = "video_etiquetas.mp4"  # Cambiar a la ruta deseada
