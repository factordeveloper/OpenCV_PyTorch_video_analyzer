    ...
    while cap.isOpened():
        ...
        if model_type == "yolo":
            ...
            with torch.no_grad():
                ...
                frame_with_results = visualize_results(frame, outputs)
...
def visualize_results(frame, outputs):
    """
    ...
    if isinstance(outputs, torch.Tensor):
        ...
        # Dibujar etiquetas en el frame
        y_offset = 30
        for i, class_idx in enumerate(top_classes):
            class_prob = probs[class_idx]
            text = f"Clase {class_idx}: {class_prob:.2f}"
            cv2.putText(frame, text, (10, y_offset * (i+1)), 
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    
    # Ejemplo para un modelo de detección de objetos:
    # (Debes adaptar esta parte según la salida de tu modelo)
